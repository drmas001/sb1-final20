    ...
    if (viewMode === 'specialty') {
      specialties.forEach((specialty) => {
        ...
        activePatients.forEach(patient => {
          content += `- ${patient.name} (Age: ${patient.age}, Admitted: ${new Date(patient.admissionDate).toLocaleString()})\n`;
        });
        content += `Discharged Patients:\n`;
        dischargedPatients.forEach(patient => {
          content += `- ${patient.name} (Age: ${patient.age}, Discharged: ${new Date(patient.dischargeDate!).toLocaleString()})\n`;
        });
        content += '\n';
      });
    } else {
      ...
      content += `Active Patients:\n`;
      activePatients.forEach(patient => {
        content += `- ${patient.name} (Age: ${patient.age}, Specialty: ${patient.specialty}, Admitted: ${new Date(patient.admissionDate).toLocaleString()})\n`;
      });
      content += `\nDischarged Patients:\n`;
      dischargedPatients.forEach(patient => {
        content += `- ${patient.name} (Age: ${patient.age}, Specialty: ${patient.specialty}, Discharged: ${new Date(patient.dischargeDate!).toLocaleString()})\n`;
      });
    }
                ...
                <ul className="space-y-2">
                  {activePatients.map((patient) => (
                    <li key={`active-${patient.id}`} className="bg-green-50 p-2 rounded-md text-green-800">
                      {patient.name} (Age: {patient.age}, Admitted: {new Date(patient.admissionDate).toLocaleString()})
                    </li>
                  ))}
                </ul>
              </div>
                ...
                <ul className="space-y-2">
                  {dischargedPatients.map((patient) => (
                    <li key={`discharged-${patient.id}`} className="bg-gray-50 p-2 rounded-md text-gray-800">
                      {patient.name} (Age: {patient.age}, Discharged: {new Date(patient.dischargeDate!).toLocaleString()})
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <ul className="space-y-2">
              {activePatients.map((patient) => (
                <li key={`active-${patient.id}`} className="bg-green-50 p-2 rounded-md text-green-800">
                  {patient.name} (Age: {patient.age}, Specialty: {patient.specialty}, Admitted: {new Date(patient.admissionDate).toLocaleString()})
                </li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-2">
              {dischargedPatients.map((patient) => (
                <li key={`discharged-${patient.id}`} className="bg-gray-50 p-2 rounded-md text-gray-800">
                  {patient.name} (Age: {patient.age}, Specialty: {patient.specialty}, Discharged: {new Date(patient.dischargeDate!).toLocaleString()})
                </li>
              ))}
            </ul>
          </div>
